...
?>	
						<tr class="tr-interno">
							<td class="botoes-interno"><a href='<?php echo $configUrl; ?>cadastros/balnearioGaivota/alterar/<?php echo $dadosNomeBalnearioGaivota['codBalnearioGaivota'] ?>/' title='Deseja alterar <?php echo $dadosNomeBalnearioGaivota['nomeBalnearioGaivota'] ?>?' ><img src="<?php echo $configUrl;?>f/i/default/corpo-default/icones-alterar-branco.gif" alt="icone" /></a></td>
						</tr>
					</table>	
					<div class="botao-consultar"><a title="Consultar Balneário Gaivota" href="<?php echo $configUrl;?>cadastros/balnearioGaivota/"><div class="esquerda-consultar"></div><div class="conteudo-consultar">Consultar</div><div class="direita-consultar"></div></a></div>					
				</div>
...
<?php
				...
				if($_POST['cod-imagem'] !=  ""){
					$sqlUpdate = "UPDATE balnearioGaivotaImagens SET capaBalnearioGaivotaImagem = 'F' WHERE codBalnearioGaivota = ".$url[6]."";
...
-->
						...
						<style>
							...
							#carregamento-upload .texto {font-size:16px; color:#666; margin-top:65px; padding-top:50px; font-family:Arial; text-align:center; background:transparent url('<?php echo $configUrlGer;?>cadastros/balnearioGaivota/loading.gif') center top no-repeat;}
						</style>
						...
						<form enctype="multipart/form-data" method="POST" action="<?php echo $configUrlGer;?>cadastros/balnearioGaivota/upload.php" onSubmit="return false loading()">
...
-->
						<br/>
						<br/>
						<form action="<?php echo $configUrlGer; ?>cadastros/balnearioGaivota/imagens/<?php echo $url[6]; ?>/" enctype="multipart/form-data" method="post">
							<div class="lista-imagens">
<?php
				$sqlRegistro = "SELECT count(codBalnearioGaivotaImagem) registros FROM balnearioGaivotaImagens WHERE codBalnearioGaivota = ".$url[6]."";
				...
				$pagina = $url[7];
				if($pagina == 1 || $pagina == "" ){
					$sqlConsulta = "SELECT * FROM balnearioGaivotaImagens WHERE codBalnearioGaivota = ".$url[6]." ORDER BY capaBalnearioGaivotaImagem ASC, codBalnearioGaivotaImagem ASC LIMIT 0, 30";
					$somaLimite = "30";
					$pgInicio = "0";		
				}else{
					$somaLimite = $pagina * 30;
					$pgInicio = $somaLimite - 30;
					$sqlConsulta = "SELECT * FROM balnearioGaivotaImagens WHERE codBalnearioGaivota = ".$url[6]." ORDER BY capaBalnearioGaivotaImagem ASC, codBalnearioGaivotaImagem ASC LIMIT ".$pgInicio.", 30";
				}
				...
				while($dadosImagem = $resultConsulta->fetch_assoc()){
					...
					if($dadosImagem['extBalnearioGaivotaImagem'] == "svg"){
?>       
								<div class="imagem-sem" style="width:180px; float:left; margin-right:20px; margin-bottom:20px; height:160px;"><p class="imagem-pequena" style="width:180px; height:134px; overflow:hidden; margin-bottom:10px;"><img style="display:table; margin:0 auto;" id="imagem-imovel<?php echo $contImagem;?>" src="<?php echo $configUrlGer; ?>f/balnearioGaivota/<?php echo $dadosImagem['codBalnearioGaivota'].'-'.$dadosImagem['codBalnearioGaivotaImagem'].'-O.'.$dadosImagem['extBalnearioGaivotaImagem'];?>" alt="Imagem BalnearioGaivota" width="100"/></p>
...
?>
								<div class="imagem-sem" style="width:180px; float:left; margin-right:20px; margin-bottom:20px; height:200px;"><p class="imagem-pequena" style="width:180px; height:180px; overflow:hidden; margin-bottom:10px;"><img style="display:table; margin:0 auto;" id="imagem-imovel<?php echo $contImagem;?>" src="<?php echo $configUrlGer; ?>f/balnearioGaivota/<?php echo $dadosImagem['codBalnearioGaivota'].'-'.$dadosImagem['codBalnearioGaivotaImagem'].'-W.webp';?>" alt="Imagem BalnearioGaivota" width="100%"/></p>
								<label class="excluir" style="float:left; cursor:pointer;"><input style="cursor:pointer;" type="checkbox" name="excluir<?php echo $cont; ?>" value="<?php echo $dadosImagem['codBalnearioGaivotaImagem']; ?>" /> Excluir</label>				
								<label class="capa" style="float:right; cursor:pointer;">Capa <input style="cursor:pointer;" type="radio" onClick="capaSubmit(<?php echo $dadosImagem['codBalnearioGaivotaImagem'];?>);" name="capa" value="<?php echo $dadosImagem['codBalnearioGaivotaImagem']; ?>" <?php echo $dadosImagem['capaBalnearioGaivotaImagem'] == 'T' ? 'checked' : '';?>/></label><br class="clear"/></div>								
<?php
					}
...
?>	
 ...
 <?php
				...
				$regPorPag = 30;
				$area = "cadastros/balnearioGaivota/imagens/".$url[6];
...
?>
					...
					<form id="submit2" action="<?php echo $configUrlGer; ?>cadastros/balnearioGaivota/imagens/<?php echo $url[6]; ?>/1/" method="post">
