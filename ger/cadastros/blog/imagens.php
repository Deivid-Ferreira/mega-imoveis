...
?>	
						<tr class="tr-interno">
							<td class="botoes-interno"><a href='<?php echo $configUrl; ?>cadastros/blog/alterar/<?php echo $dadosNomeBlog['codBlog'] ?>/' title='Deseja alterar <?php echo $dadosNomeBlog['nomeBlog'] ?>?' ><img src="<?php echo $configUrl;?>f/i/default/corpo-default/icones-alterar-branco.gif" alt="icone" /></a></td>
						</tr>
					</table>	
					<div class="botao-consultar"><a title="Consultar Blog" href="<?php echo $configUrl;?>cadastros/blog/"><div class="esquerda-consultar"></div><div class="conteudo-consultar">Consultar</div><div class="direita-consultar"></div></a></div>					
				</div>
						...
						for($i=0; $i<=$_POST['cont']; $i++){
							...
							if(isset($_POST[$contadorDel])){
								...
								if(file_exists("f/blog/".$dadosConsultaDelete['codBlog']."-".$dadosConsultaDelete['codBlogImagem']."-O.".$dadosConsultaDelete['extBlogImagem'])){
									...
									unlink("f/blog/".$dadosConsultaDelete['codBlog']."-".$dadosConsultaDelete['codBlogImagem']."-P.".$dadosConsultaDelete['extBlogImagem']);
									unlink("f/blog/".$dadosConsultaDelete['codBlog']."-".$dadosConsultaDelete['codBlogImagem']."-M.".$dadosConsultaDelete['extBlogImagem']);
									unlink("f/blog/".$dadosConsultaDelete['codBlog']."-".$dadosConsultaDelete['codBlogImagem']."-G.".$dadosConsultaDelete['extBlogImagem']);
								}
...
-->
						...
						<style>
							...
							#carregamento-upload .texto {font-size:16px; color:#666; margin-top:65px; padding-top:50px; font-family:Arial; text-align:center; background:transparent url('<?php echo $configUrlGer;?>cadastros/blog/loading.gif') center top no-repeat;}
						</style>
						...
						<form enctype="multipart/form-data" method="POST" action="<?php echo $configUrlGer;?>cadastros/blog/upload.php" onSubmit="return false loading()">
							...
							<label class="label" style="font-size:15px; line-height:30px; font-weight:normal;"><strong>Extensão:</strong> png,jpg ou jpeg | <strong>Tamanho:</strong> 800 x 600<br/><input style="display:block; margin-right:20px; float:left;" type="file" class="campo" id="arquivo" name="arquivo[]" multiple="multiple" required /></labeL>
...
-->
						<br/>
						<br/>
						<form action="<?php echo $configUrlGer; ?>cadastros/blog/imagens/<?php echo $url[6]; ?>/" enctype="multipart/form-data" method="post">
...
?>	
 ...
 <?php
				...
				$regPorPag = 30;
				$area = "cadastros/blog/imagens/".$url[6];
...
?>
					...
					<form id="submit2" action="<?php echo $configUrlGer; ?>cadastros/blog/imagens/<?php echo $url[6]; ?>/1/" method="post">
