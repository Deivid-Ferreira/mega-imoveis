		...
		if($controleUsuario == "tem usuario"){
			...
			if($validaAcesso == "ok"){
				...
				if($_SESSION['alteracao'] == "ok"){
					$erroConteudo = "<p class='erro'>Blog <strong>".$_SESSION['nome']."</strong> alteradao com sucesso!</p>";
					$_SESSION['alteracao'] = "";
					$_SESSION['nome'] = "";
				}else	
				...
				<div id="filtro">
					...
					<div class="demoTarget">
						<div id="formulario-filtro">
							...
							<form name="filtro" action="<?php echo $configUrl;?>cadastros/blog/" method="post" />
								<div class="botao-novo" style="margin-left:0px;"><a title="Novo Blog" onClick="abreCadastrar()"><div class="esquerda-novo"></div><div class="conteudo-novo">Novo Blog</div><div class="direita-novo"></div></a></div>
				...
				if(isset($_POST['cadastrar']) || isset($_POST['cadastrarFotos'])){
					...
					if($result == 1){
						if(isset($_POST['cadastrar'])){
							...
							echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=".$configUrlGer."cadastros/blog/'>";
						}else{
							...
							echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=".$configUrlGer."cadastros/blog/imagens/".$dadosBlog['codBlog']."/'>";
						}
					}else{
						$erroData = "<p class='erro'>Problemas ao cadastrar blog!</p>";
					}
...
?>				
						...
						<br/>
						<form action="<?php echo $configUrlGer; ?>cadastros/blog/" method="post">
...
?>
								<tr class="tr">
									<td class="setenta"><a href='<?php echo $configUrlGer; ?>cadastros/blog/alterar/<?php echo $dadosBlog['codBlog'] ?>/' title='Veja os detalhes do blog <?php echo $dadosBlog['nomeBlog'] ?>'><?php echo $dadosBlog['nomeBlog'];?></a></td>
									<td class="vinte" style="text-align:center;"><a style="padding:0px;" href='<?php echo $configUrlGer; ?>cadastros/blog/alterar/<?php echo $dadosBlog['codBlog'] ?>/' title='Veja os detalhes do blog <?php echo $dadosBlog['nomeBlog'] ?>'><?php echo $dataProgramada;?></a></td>
									<td class="dez" style="text-align:center;"><a href='<?php echo $configUrlGer; ?>cadastros/blog/alterar/<?php echo $dadosBlog['codBlog'] ?>/' title='Veja os detalhes do blog <?php echo $dadosBlog['nomeBlog'] ?>'><?php echo data($dadosBlog['dataBlog']);?></a></td>
									<td class="botoes" style="width:5%;"><a style="padding:0px;" href='<?php echo $configUrl; ?>cadastros/blog/imagens/<?php echo $dadosBlog['codBlog'] ?>/' title='Deseja gerenciar imagens do blog <?php echo $dadosBlog['nomeBlog'] ?>?' ><img style="<?php echo $dadosImagem['codBlogImagem'] == "" ? 'display:none;' : 'padding-top:5px;';?>" src="<?php echo $configUrlGer.'f/blog/'.$dadosImagem['codBlog'].'-'.$dadosImagem['codBlogImagem'].'-P.'.$dadosImagem['extBlogImagem'];?>" height="50"/><img style="<?php echo $dadosImagem['codBlogImagem'] != "" ? 'display:none;' : '';?>" src="<?php echo $configUrl; ?>f/i/default/corpo-default/gerenciar-imagens.gif" alt="icone"></a></td>
									<td class="botoes"><a href='<?php echo $configUrl; ?>cadastros/blog/ativacao/<?php echo $dadosBlog['codBlog'] ?>/' title='Deseja <?php echo $statusPergunta ?> o blog <?php echo $dadosBlog['nomeBlog'] ?>?' ><img src="<?php echo $configUrl; ?>f/i/default/corpo-default/<?php echo $status ?>.gif" alt="icone"></a></td>
									<td class="botoes"><a href='<?php echo $configUrl; ?>cadastros/blog/alterar/<?php echo $dadosBlog['codBlog'] ?>/' title='Deseja alterar o blog <?php echo $dadosBlog['nomeBlog'] ?>?' ><img src="<?php echo $configUrl;?>f/i/default/corpo-default/icones-alterar.gif" alt="icone" /></a></td>
								...
								<script>
									function confirmaExclusao(cod, nome){
										if(confirm("Deseja excluir o blog "+nome+"?")){
											window.location='<?php echo $configUrlGer; ?>cadastros/blog/excluir/'+cod+'/';
										}
									}
								</script>
