						...
						for($i=0; $i<=$_POST['cont']; $i++){
							...
							if(isset($_POST[$contadorDel])){
								...
								if(file_exists("f/clientes/".$dadosConsultaDelete['codCliente']."-".$dadosConsultaDelete['codClienteImagem']."-O.".$dadosConsultaDelete['extClienteImagem'])){
									...
									unlink("f/clientes/".$dadosConsultaDelete['codCliente']."-".$dadosConsultaDelete['codClienteImagem']."-P.".$dadosConsultaDelete['extClienteImagem']);
									unlink("f/clientes/".$dadosConsultaDelete['codCliente']."-".$dadosConsultaDelete['codClienteImagem']."-M.".$dadosConsultaDelete['extClienteImagem']);
									unlink("f/clientes/".$dadosConsultaDelete['codCliente']."-".$dadosConsultaDelete['codClienteImagem']."-G.".$dadosConsultaDelete['extClienteImagem']);
								}
						...
						if(in_array($ext, ['jpg', 'jpeg', 'png', 'gif'])){	
							...
							if($result == 1){
								...
								foreach ($sizes as $size) {
									list($width, $height) = getimagesize($pastaDestino.$codCliente."-".$codClienteImagem."-O.".$ext);
									...
									switch ($ext) {
										case 'jpg':
										case 'jpeg':
										imagejpeg($new_image, $pastaDestino.$codCliente."-".$codClienteImagem."-".$tamanho.".".$ext, 100);
										break;
										case 'png':
										imagepng($new_image, $pastaDestino.$codCliente."-".$codClienteImagem."-".$tamanho.".".$ext);
										break;
										case 'gif':
										imagegif($new_image, $pastaDestino.$codCliente."-".$codClienteImagem."-".$tamanho.".".$ext);
										break;
									}
									...
									chmod ($pastaDestino.$codCliente."-".$codClienteImagem."-P.".$ext, 0755);
									chmod ($pastaDestino.$codCliente."-".$codClienteImagem."-M.".$ext, 0755);
									chmod ($pastaDestino.$codCliente."-".$codClienteImagem."-G.".$ext, 0755);								
								}
...
?>				
						...
						<form action="<?php echo $configUrlGer; ?>imoveis/clientes/foto/<?php echo $url[6]; ?>/1/" enctype="multipart/form-data" method="post">
...
?>
								...
								<p class="imagem"><img src="<?php echo $configUrlGer; ?>f/clientes/<?php echo $dadosImagem['codCliente'].'-'.$dadosImagem['codClienteImagem'].'-P.'.$dadosImagem['extClienteImagem'];?>" alt="Imagem Cliente" /><br/>
...
<?php
				...
				$regPorPag = 14;
				$area = "imoveis/clientes/foto/".$url[6];
