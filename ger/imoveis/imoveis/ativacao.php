	...
	if($_COOKIE['loginAprovado'.$cookie] != ""){
		...
		if($controleUsuario == "tem usuario"){
			...
			if($validaAcesso == "ok"){
				...
				if($url[5] != ""){
					$sqlCons = "SELECT nomeImovel, statusImovel FROM imoveis WHERE codImovel = ".$url[6]." LIMIT 0,1";
					$resultCons = $conn->query($sqlCons);
					$dadosCons = $resultCons->fetch_assoc();
					
					echo "<div id='filtro'>";
					
					if($dadosCons['statusImovel'] == "T"){
					...
					$sql =  "UPDATE imoveis SET statusImovel = '".$alteraStatus."' WHERE codImovel = ".$url[6];
