...
?>	
						<tr class="tr-interno">
							<td class="botoes-interno"><a href='<?php echo $configUrl; ?>imoveis/imoveis/alterar/<?php echo $dadosNomeImovel['codImovel'] ?>/' title='Deseja alterar o imóvel <?php echo $dadosNomeImovel['nomeImovel'] ?>?' ><img src="<?php echo $configUrl;?>f/i/default/corpo-default/icones-alterar-branco.gif" alt="icone" /></a></td>
						</tr>
					</table>	
...
-->
						...
						<style>
							...
							#carregamento-upload .texto {font-size:16px; color:#666; margin-top:65px; padding-top:50px; font-family:Arial; text-align:center; background:transparent url('<?php echo $configUrlGer;?>imoveis/imoveis/loading.gif') center top no-repeat;}
						</style>
						...
						<form enctype="multipart/form-data" method="POST" action="<?php echo $configUrlGer;?>imoveis/imoveis/upload.php" onSubmit="return false loading()">
							...
							<label class="label" style="font-size:15px; line-height:30px; font-weight:normal;"><strong>Extensão:</strong> png, jpg, jpeg, gif ou mp4<br/><strong>Tamanho  Mímino:</strong> 600 x 338 | <strong>Tamanho  Recomendado:</strong> 1920 x 1080<br/><input style="display:block; margin-right:20px; float:left; width:430px;" type="file" class="campo" id="arquivo" name="arquivo[]" multiple="multiple" required /></labeL>
...
-->
						...
						<script type="text/javascript">
							function salvaOrdenacao(ordenacao, cod){
								...
								$s.post("<?php echo $configUrlGer;?>imoveis/imoveis/salva-ordenacao.php", {codImovelImagem: cod, ordenacaoImovelImagem: ordenacao},function(data){  
								});  
							} 					
						</script>
						<form action="<?php echo $configUrlGer; ?>imoveis/imoveis/imagens/<?php echo $url[6]; ?>/" enctype="multipart/form-data" method="post">
...
?>
								<div class="imagem-sem" style="width:180px; float:left; margin-right:20px; margin-bottom:20px; height:160px;">
									<p class="imagem-pequena" style="width:180px; height:101px; overflow:hidden; margin-bottom:10px;">
										<a rel="lightbox[roadtrip]"  href="<?php echo $configUrlGer; ?>f/imoveis/<?php echo $dadosImagem['codImovel'].'-'.$dadosImagem['codImovelImagem'].'-W.webp';?>">
										    <img id="imagem-imovel<?php echo $contImagem;?>" src="<?php echo $configUrlGer; ?>f/imoveis/<?php echo $dadosImagem['codImovel'].'-'.$dadosImagem['codImovelImagem'].'-W.webp';?>"  alt="Imagem Imovel" width="180"/>
										</a>
									</p>
									...
									<label style="float:right;">Ordenar <input style="float:right; width:15px; padding:3px; margin-top:-3px; margin-left:5px; color:#666; font-size:12px; text-align:center;" type="text" onKeyup="salvaOrdenacao(this.value, <?php echo $dadosImagem['codImovelImagem'];?>);" value="<?php echo $dadosImagem['ordenacaoImovelImagem'];?>" name="ordenacao" id="ordenacao"/></label>
								</div>				
<?php
					}else{
?>
...
?>	
 ...
 <?php
				...
				$regPorPag = 30;
				$area = "imoveis/imoveis/imagens/".$url[6];
...
?>
					...
					<form id="submit2" action="<?php echo $configUrlGer; ?>imoveis/imoveis/imagens/<?php echo $url[6]; ?>/1/" method="post">
