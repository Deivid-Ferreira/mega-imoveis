	...
	if($_COOKIE['loginAprovado'.$cookie] != ""){
		...
		if($controleUsuario == "tem usuario"){
			...
			if($validaAcesso == "ok"){
				...
				if($url[5] != ""){
					$sqlCons = "SELECT nomeCidade, statusCidade FROM cidades WHERE codCidade = ".$url[6]." LIMIT 0,1";
					...
					$sql =  "UPDATE cidades SET statusCidade = '".$alteraStatus."' WHERE codCidade = ".$url[6];
