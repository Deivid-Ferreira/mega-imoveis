	...
	if($_COOKIE['loginAprovado'.$cookie] != ""){
		...
		if($controleUsuario == "tem usuario"){
			...
			if($validaAcesso == "ok"){
				...
				if($url[5] != ""){
					$sqlCons = "SELECT nomeQuadra, statusQuadra FROM quadras WHERE codQuadra = ".$url[6]." LIMIT 0,1";
					$resultCons = $conn->query($sqlCons);
					$dadosCons = $resultCons->fetch_assoc();
					
					echo "<div id='filtro'>";
					
					if($dadosCons['statusQuadra'] == "T"){
					...
					$sql =  "UPDATE quadras SET statusQuadra = '".$alteraStatus."' WHERE codQuadra = ".$url[6];
