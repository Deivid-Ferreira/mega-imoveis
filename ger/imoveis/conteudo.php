...
<?php
	...
	if($url[$urlNumber] == 1 || $url[$urlNumber] == ""){
		$pagina = 1;
		$sqlImoveis = "SELECT DISTINCT I.* FROM imoveis I inner join imoveisImagens II on I.codImovel = II.codImovel inner join tipoImovel TI on I.codTipoImovel = TI.codTipoImovel inner join cidades C on I.codCidade = C.codCidade inner join bairros B on I.codBairro = B.codBairro WHERE I.statusImovel = 'T'".$filtraCodigo.$filtraImoveis.$filtraCodigo.$filtraImovel.$filtraNegociacao.$filtraDormitorio.$filtraSuites.$filtraBanheiros.$filtraVagas.$filtraTipoV.$filtraCidade.$filtraBairro.$filtraCodigo.$filtraPreco." ORDER BY".$ordenar." I.lancamentoImovel ASC, I.destaqueImovel ASC, I.codImovel DESC LIMIT 0,16";
	}else{
		$pagina = $url[$urlNumber];
		$paginaFinal = $pagina * 16;
		$paginaInicial = $paginaFinal - 16;
		$sqlImoveis = "SELECT DISTINCT I.* FROM imoveis I inner join imoveisImagens II on I.codImovel = II.codImovel inner join tipoImovel TI on I.codTipoImovel = TI.codTipoImovel inner join cidades C on I.codCidade = C.codCidade inner join bairros B on I.codBairro = B.codBairro WHERE I.statusImovel = 'T'".$filtraCodigo.$filtraImoveis.$filtraCodigo.$filtraImovel.$filtraNegociacao.$filtraSuites.$filtraDormitorio.$filtraBanheiros.$filtraVagas.$filtraTipoV.$filtraCidade.$filtraBairro.$filtraCodigo.$filtraPreco." ORDER BY".$ordenar." I.lancamentoImovel ASC, I.destaqueImovel ASC, I.codImovel DESC LIMIT ".$paginaInicial.",16";
	}
