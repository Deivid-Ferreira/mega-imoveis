	...
	if($_COOKIE['loginAprovado'.$cookie] != ""){
		...
		if($controleUsuario == "tem usuario"){
			...
			if($validaAcesso == "ok"){
				...
				if($url[5] != ""){
					$sqlCons = "SELECT nomeAnuncie, statusAnuncie FROM anuncie WHERE codAnuncie = ".$url[6]." LIMIT 0,1";
					$resultCons = $conn->query($sqlCons);
					$dadosCons = $resultCons->fetch_assoc();
					echo "<div id='filtro'>";
					
					if($dadosCons['statusAnuncie'] == "T"){
					...
					$sql =  "UPDATE anuncie SET statusAnuncie = '".$alteraStatus."' WHERE codAnuncie = ".$url[6];
					$result = $conn->query($sql);
					
					if($result == 1){
						$_SESSION['nome'] = $dadosCons['nomeAnuncie'];
						$_SESSION['ativar'] = "ok";
						echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=".$configUrlGer."atendimento/anuncie/'>";
					}
